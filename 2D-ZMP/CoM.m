...
% CoM.P = [CoM.AbP CoM.AbRA CoM.AbRT CoM.AbRF CoM.AbLF CoM.AbLT CoM.AbLA];
% CoM.m = [param.mP;param.mA;param.mT;param.mF;param.mF;param.mT;param.mA];
% CoM.M = sum(CoM.m);
% CoM.Output = (1/CoM.M) * sum(repmat(CoM.m,3,1) .* CoM.P,2);
...
elseif model.base == -1    
    ...
    CoM.AbRA = HT.AbRB + HTf(-param.A/2,0,0);
    CoM.AbRT = HT.Ab3 + Rz(q(3))*HTf(0,param.T/2,0);
    CoM.AbRF = HT.Ab2 + Rz(q(2) + q(3))*HTf(0,param.F/2,0);
    CoM.AbLF = HT.Ab5 + Rz(-q(5) - q(6))*HTf(0,param.F/2,0);
    CoM.AbLT = HT.Ab6 + Rz(-q(6))*HTf(0,param.T/2,0);
    CoM.AbLA = HT.AbLB + HTf(-param.A/2,0,0);
