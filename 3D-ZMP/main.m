...
plot(model.tspan, model.CoM_star(1,:),'linewidth',3);
grid on
hold on 
plot(model.tspan,ZMPpc(1,:),'linewidth',3);
plot(model.tspan,positionCoM(1,:),'linewidth',3);
plot(model.tspan,CoMpc(1,:),'--','linewidth',3);
legend("ZMP Reference","ZMP","CoM-Ref","CoM",'location','best');
xlabel('Time (sec)');
ylabel('X-Axis (m)');
title('3D ZMP and CoM Trajectory Tracking X-axis')
axis([     1.75,        4, ... % Z
           0,        0.3]);   % Y
saveas(fig2,'ZMP_3D_x.png')

%%
fig3 = figure;
set(fig3,'Position', [100,100,800,400])
plot(model.tspan, model.CoM_star(2,:),'linewidth',3);
grid on
hold on 
plot(model.tspan,ZMPpc(2,:),'linewidth',3);
plot(model.tspan,positionCoM(3,:),'linewidth',3);
plot(model.tspan,CoMpc(4,:),'--','linewidth',3);
legend("ZMP Reference","ZMP","CoM-Ref","CoM",'location','best');
xlabel('Time (sec)');
ylabel('Y-Axis (m)');
title('3D ZMP and CoM Trajectory Tracking Y-axis')
saveas(fig3,'ZMP_3D_y.png')
